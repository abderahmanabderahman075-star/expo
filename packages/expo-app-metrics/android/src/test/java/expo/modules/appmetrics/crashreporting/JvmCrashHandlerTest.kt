...
class JvmCrashHandlerTest {
  ...
  @Test
  fun `writes a pending crash file with the provided session id`() {
    val writer = writer()
    val handler = JvmCrashHandler(writer, sessionIdProvider = { "session-1" }, previousHandler = null)
  ...
  fun `records a null session id when the identity is not ready yet`() {
    val writer = writer()
    val handler = JvmCrashHandler(writer, sessionIdProvider = { null }, previousHandler = null)
  ...
  fun `always delegates to the previous handler`() {
    ...
    val handler = JvmCrashHandler(writer(), { "session-1" }, previous)
  ...
  fun `delegates even when writing the crash file fails`() {
    ...
    val previous = RecordingHandler()
    val handler = JvmCrashHandler(CrashFileWriter(blocked), { "session-1" }, previous)
    val throwable = IllegalStateException("boom")
  ...
  @Test
  fun `delegates even when the session id provider throws`() {
    val previous = RecordingHandler()
    val handler = JvmCrashHandler(writer(), { error("provider blew up") }, previous)
    val throwable = IllegalStateException("boom")
  ...
  fun `install chains to whatever handler was registered before`() {
    ...
    JvmCrashHandler.install(writer()) { "session-1" }
  ...
  fun `install is idempotent`() {
    ...
    JvmCrashHandler.install(writer()) { "session-2" }
  ...
  fun `does not throw without a previous handler`() {
    val handler = JvmCrashHandler(writer(), { "session-1" }, previousHandler = null)
