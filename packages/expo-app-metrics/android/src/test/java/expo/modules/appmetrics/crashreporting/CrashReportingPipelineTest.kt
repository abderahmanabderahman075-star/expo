 ...
 * real [SessionCrashReportSink] → real Room storage → real JS mapper. Each link
 * is unit-tested in isolation; this pins the contracts *between* them (field
  ...
  fun `a JVM crash surfaces in getInactiveSessions on the next launch`() =
    runTest {
      ...
      CrashReportProcessor(
        ...
        appVersion = "3.1.4",
        crashReportSink = SessionCrashReportSink(sessionManager) { currentSessionId }
      ).process()
