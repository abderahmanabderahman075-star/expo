 ...
 * No JSON, no database, no coroutines; the structured [CrashReport] is assembled
 * by [CrashFileReader] from the parsed file on the next launch, where allocation
 * is safe.
 *
...
class CrashFileWriter(
  ...
  /**
   * Creates the crash directory off the calling thread. Called at install from
   * `Application.onCreate()` on the main thread. Returns the [Job]; callers need not await it — the crash
   * path self-heals via [write]'s own `mkdirs()` fallback, so this only spares the common
   * write case the directory-create I/O.
   *
   * `mkdirs()` is wrapped in `runCatching`: a failure (e.g. `SecurityException`) must not escape
   * the coroutine, or it would reach the thread's uncaught-exception handler — which is the very
   * [JvmCrashHandler] we just installed. [write] retries the create on the crash path anyway.
   */
