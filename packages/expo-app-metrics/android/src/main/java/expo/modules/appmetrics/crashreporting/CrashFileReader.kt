...
class CrashFileReader(private val directory: File) {
  ...
  fun listPendingCrashes(): List<PendingJvmCrash> {
    val files = directory.listFiles { file -> CrashFileFormat.FILE_NAME_PATTERN.matches(file.name) } ?: return emptyList()
  ...
  companion object {
    /** The canonical pending-crash location — must match [CrashFileWriter.forContext]. */
    fun forContext(context: android.content.Context): CrashFileReader =
      CrashFileReader(java.io.File(context.noBackupFilesDir, "expo-app-metrics/crashes"))
  }
}
